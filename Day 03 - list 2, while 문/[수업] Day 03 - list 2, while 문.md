...
- 무한 루프
  ...
  - Iteration 이 무한히 도는 상황을 말하는데, 보통은 좋은 상황은 아니다. 특별히 무한 루프를 내가 만든 것이 아닌 한에서는 거의 대부분 뭔가 내가 코드를 잘못짰기 때문에 발생한다. 무한 루프의 더 큰 문제는...... 무한 루프가 돌고있다는 상황 자체를 인지하기 어렵기 때문이다! (그것도 모른 채 코드는 계속 무한으로 끝없이 돌고있으면.. 정말 답답한 상황) 내 코드가 단순히 좀 늦게 돌아가는 코드인지, 아니면 무한루프가 돌아가고 있는 것인지 그걸 잘 파악해야 코드를 금방 금방 고칠 수 있다.
  ...
  - 무한 루프를 안만드는게 최선이지만, 은근 흔하게 실수하다 많이 만들어진다. 그래서 **무한루프인지 빨리 파악**하는게 정말 정말 중요하다. 여러 방법이 있겠지만, iteration마다 iterator를 프린트 해줘서, iterator가 제때 업데이트 되는지 확인하는 방법도 있다.
    ...
    - 예) 매 iteration마다 너무 자주 프린트를 하면 (스크롤 압박이 심하고) 코드의 성능이 떨어질 수 있기에, 대충 10번, 100번, 1000번째 (자기가 고르기 나름) iteration마다 프린트 하기도 한다
